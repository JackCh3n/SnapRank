...
- **归档对象为源图**（复制/移动原始照片；压缩图仅用于评分输入与预览，保证"精选"档可直接冲印）。
- **归档方式**：默认**复制**；"移动"需在确认弹窗中显式选择（确认后才会移动源图，默认源图安全不动）。
...
- Prompt 评分锚点对齐国际摄影比赛（PSA/FIAP）评审尺度（v3，从严）：
  `9-10 获奖级（极罕见）；8-8.9 入选级（accepted）；7-7.9 良好（缺乏亮点）；5-6.9 一般（明显缺陷或构图平淡）；3-4.9 差（多处硬伤）；0-2.9 废片`。
  评审纪律：默认从严、分数必须"赢得"而非"证明没有问题"、禁止全部落入 6-8 舒适区。
- 思考型模型（glm-5.3 等）的推理计入 `max_tokens`：默认 2048；输出为空且 finish_reason=length 时明确报错提示调大。
